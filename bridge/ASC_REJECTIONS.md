...
## 所見: 6813xxx 帯は同一テンプレ由来か

> 件数の訂正: 指示は「6813xxx 帯 12 件」だったが、REJECTED 14 件のうち App ID が `6813` で
> 始まるのは実際には **13 件**（帯の外は `6773762868` Pulsebound の 1 件のみ）。以下は 13 件の所見。

1. **指摘内容が共通かは判定不能。** 却下本文が公開 API に無く（14/14 が同一 404）、比較すべき文字列が 1 件も手元に無い。
2. ただし 13 件中 **11 件**（`6813034976`〜`6813035545`）は版の作成が `2026-09-17T03:10:04`〜`03:11:18 -07:00` の **74 秒間**、再提出が `2026-09-18T01:30:47`〜`01:35:09Z` の **4 分 22 秒間**に収まり、一括生成・一括提出された同一テンプレ由来とみて矛盾しない。
3. 残り 2 件は時刻が外れる（`6813190990` は提出 09-17T20:28、`6813758271` は 09-19T05:11）ため、同じ波の一部とは言い切れない。
4. 11 件は bundleId も `jp.wch.<英単語>` で揃い、状態も全件 `REJECTED` + `UNRESOLVED_ISSUES` + item `REJECTED` と完全一致。**状態の共通性は実データで確認済み**だが、これは「同じ理由で却下された」ことの証明にはならない。
5. 断定には本文が要る。`node ~/scripts/asc/iris-snippets.mjs rejections <13 件の ID>` をログイン済み ASC タブで実行すれば比較できる（本セッションは手段が無く未実行）。
